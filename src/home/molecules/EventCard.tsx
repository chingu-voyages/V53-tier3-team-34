...
const EventCard: React.FC<EventCardProps> = ({
  event: { id, title, imageUrl, startDateTime, address, costPerPerson },
  textColor,
}) => {
  return (
    <Link href={`/events/${id}`}>
      ...
      >
        ...
        <h2
          ...
          } text-xl leading-loose break-words ${monaSans.className}`}
        >
          {title}
        </h2>
        <div
        ...
        <div
          className={`${
            textColor ? textColor : "text-black"
          } text-base leading-loose break-words ${inter.className}`}
        >
          {address}
        </div>
        <p
